#' Plot DAG or Heatmap of Adjacency Matrix
#'
...
#' @param type Type of plot to display. Either \code{"dag"} (default) for a directed graph with arrows or \code{"heatmap"} for a matrix view.
#' @param layout Layout algorithm to use for node positioning when \code{type = "dag"}. Options include \code{"sugiyama"}, \code{"kk"}, \code{"fr"}, etc. Passed to \code{ggraph()}.
#' @param show_legend Logical. If \code{TRUE}, displays a legend for node/edge elements. Default is \code{FALSE}.
#' @param edge_color Color of the DAG edges (arrows). Default is \code{"black"}.
#' @param node_color Fill color of the DAG nodes. Default is \code{"steelblue"}.
#' @param label_color Color of the text labels inside the nodes. Default is \code{"white"}.
#' @param arrow_size Size of the DAG arrow heads (in mm units). Default is \code{4}.
#'
#' @return A \code{ggplot} object representing either the DAG or the adjacency matrix.
#'
#' @details
...
#'   \item A \strong{DAG plot} using the \code{ggraph} and \code{igraph} packages.
#' }
#'
#' The DAG layout uses standard force-directed or layered layout algorithms such as
#' \code{"sugiyama"} or \code{"kk"} to organize nodes and arrows cleanly.
#'
...
#' \dontrun{
#' res <- mdm(y)
#' plot_dag2(res, type = "dag", layout = "sugiyama")
#' plot_dag2(res, type = "heatmap")
#' }
plot_dag2 <- function(mdm_object, node_labels = NULL,
                      type = c("heatmap", "dag"),
                      layout = "sugiyama",
                      show_legend = FALSE,
                      edge_color = "black",
